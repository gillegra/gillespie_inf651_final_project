...
const toggleCommentButton = (postId) => {
  ...
  if (button) {
    button.textContent === "Show Comments" ? button.textContent = "Hide Comments" : "Show Comments";
  }
  return button;
}
...
const deleteChildElements = (parentElement) => {
  ...
  while ((childElement = parentElement.lastElementChild ?? null) && childElement) {
...
//THE FOLLOWING FUNCTIONS ARE NOT SELF-CONTAINED
...
const getUserPosts = async (userId) => {
  ...
  try {
    posts = await fetch(`https://jsonplaceholder.typicode.com/users/${userId}/posts`);
  } catch (error) {
    posts = {};
  }
